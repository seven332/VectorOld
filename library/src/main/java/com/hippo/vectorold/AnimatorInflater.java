...
public class AnimatorInflater {
    ...
    public static Animator loadAnimator(Context context, int id)
        ...
        return loadAnimator(context, id, 1);
    }

    public static Animator loadAnimator(Context context, int id, float pathErrorScale) {
        Resources resources = context.getResources();
        XmlResourceParser parser = null;
        try {
            parser = resources.getAnimation(id);
            return createAnimatorFromXml(context, parser, pathErrorScale);
    ...
    /* TODO Not support StateListAnimator
            ...
            switch (type) {
                ...
                case XmlPullParser.START_TAG:
                    ...
                    if ("item".equals(parser.getName())) {
                        ...
                        if (animator == null) {
                            animator = createAnimatorFromXml(context, parser, 1f);
                        }
    ...
    */
    ...
    private static void setupObjectAnimator(ValueAnimator anim, TypedArray arrayObjectAnimator,
        ...
        if (pathData != null) {
            throw new IllegalStateException("Not support PropertyAnimator_pathData");
            /* TODO Not support pathData
    ...
    private static Animator createAnimatorFromXml(Context context, XmlPullParser parser,
            float pixelSize)
            throws XmlPullParserException, IOException {
        return createAnimatorFromXml(context, parser, Xml.asAttributeSet(parser), null, 0,
                pixelSize);
    }
    ...
    private static Animator createAnimatorFromXml(Context context, XmlPullParser parser,
        ...
        while (((type = parser.next()) != XmlPullParser.END_TAG || parser.getDepth() > depth)
            ...
            if (name.equals("objectAnimator")) {
                ...
                anim = loadAnimator(context, attrs, null, pixelSize);
            } else if (name.equals("set")) {
                anim = new AnimatorSet();
                TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.AnimatorSet);
                int ordering = a.getInt(R.styleable.AnimatorSet_ordering,
                        TOGETHER);
                createAnimatorFromXml(context, parser, attrs, (AnimatorSet) anim, ordering,
                        pixelSize);
                a.recycle();
            } else {
    ...
    private static ObjectAnimator loadObjectAnimator(Context context, AttributeSet attrs,
        ...
        loadAnimator(context, attrs, anim, pathErrorScale);
     ...
     *            ObjectAnimator
     */
    private static ValueAnimator loadAnimator(Context context, AttributeSet attrs,
            ValueAnimator anim, float pathErrorScale)
            throws NotFoundException {

        TypedArray arrayAnimator = context.obtainStyledAttributes(attrs, R.styleable.Animator);
        TypedArray arrayObjectAnimator = null;

        // If anim is not null, then it is an object animator.
        if (anim != null) {
            arrayObjectAnimator = context.obtainStyledAttributes(attrs,
                    R.styleable.PropertyAnimator);
        }
